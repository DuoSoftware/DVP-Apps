...
(function(){
  ...
  var EditAppController = function($scope,backendcontroller,$location,$mdDialog,$mdMedia,$mdToast, commoncontroller) {
    ...
    $scope.SaveApp= function(){
      ...
      if(editApplicationObj.ObjClass=="SYSTEM")
      {
        editApplicationObj.MasterApplicationId=null;
      }
    ...
    var onTestResult = function(response){
      ...
      if(response=="200")
      {
        ...
        commoncontroller.showAlert("Success","Application Url is working properly ");
    ...
    var onAllAppLoad= function (result) {

      console.log("All Apps "+JSON.stringify(result));

      if(result.Exception)
      {
        console.log("Gottah Error "+JSON.stringify(result));

      }
      else
      {
        $scope.AppList=result.Result;
      }
    };


    $scope.loadAllApps = function () {

      backendcontroller.getAppList().then(onAllAppLoad,onError);
    };
